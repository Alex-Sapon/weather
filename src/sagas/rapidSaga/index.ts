...
import { setRapidWeatherDataCity, setWeatherData } from '@/store/actions';
...
export function* loadForestWeather(action: ReturnType<typeof setRapidWeatherDataCity>) {
  try {
    const weather: AxiosResponse<RapidWeather> = yield call(
...
export function* stormGlassWatcher() {
  yield takeEvery('LOAD_RAPID_DATA_BASIC', loadCurrentWeather);
  yield debounce(600, 'LOAD_RAPID_DATA_CITY', loadForestWeather);
}
