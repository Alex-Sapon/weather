...
export const apiStormGlass = {
  fetchCurrentWeather(lat: number, lon: number): Promise<AxiosResponse<Omit<RapidWeather, 'forecast'>>> {
    return axios.request(options(lat, lon, 'current.json'));
  },
  fetchForecastWeather(lat: number, lon: number, days: number): Promise<AxiosResponse<RapidWeather>> {
